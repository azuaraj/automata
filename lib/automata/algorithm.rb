module Automata
    ...
    def run
        generations.times do 
            @iteration_number == 0 ? @current_arrangement_state = @seed : @current_arrangement_state
            ...
            (0...rows).each do |row|
                ...
                if @current_arrangement_state[row][col] == 1 && (live_neighbors < 2 or live_neighbors > 3)
                    @current_arrangement_state[row][col] = -1
                end

                if @current_arrangement_state[row][col] == 0 && live_neighbors == 3
